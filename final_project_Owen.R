...
for (i in 1:5){
  for (j in i:5){
    delta_spec <- sample(specified_corr[[paste0("Pair_", i, "_", j)]])[num] ## Select a random index from range of plausible values
    delta_spec_list <- c(delta_spec_list,delta_spec)
  ...
  return(list(all_data=all_data,
              delta_spec_list=delta_spec_list))
}
...
cts_data1 <- convert_to_percentiles(bivord_data1$all_data)

set.seed(123)
bivord_data2 <- generate_bivord_data(33)
cts_data2 <- convert_to_percentiles(bivord_data2$all_data)

set.seed(123)
bivord_data3 <- generate_bivord_data(50)
cts_data3 <- convert_to_percentiles(bivord_data3$all_data)

## Compute empirical correlations for each dataset
empirical_corr_list1 <- lapply(cts_data1, cor)
specified_corr_list1 <- bivord_data1$delta_spec_list
mat1 <- matrix(0, nrow = 5, ncol = 5)
mat1[upper.tri(mat1, diag = TRUE)] <- specified_corr_list1
...
specified_corr_list2 <- bivord_data2$delta_spec_list
mat2 <- matrix(0, nrow = 5, ncol = 5)
mat2[upper.tri(mat2, diag = TRUE)] <- specified_corr_list2
...
specified_corr_list3 <- bivord_data3$delta_spec_list
mat3 <- matrix(0, nrow = 5, ncol = 5)
mat3[upper.tri(mat3, diag = TRUE)] <- specified_corr_list3
mat3[lower.tri(mat3)] <- t(mat3)[lower.tri(mat3)]
print(mat3)
